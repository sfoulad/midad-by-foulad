> ## ⚠️ Historical document — pre-thin-fork BLE design/debugging record
>
> This file is preserved **verbatim** from the stale branch
> `docs/ble-phase1-hardware-validated` at commit `50e0fc64` (`docs/ble-module-tasks.md`
> at that SHA), captured during the read-only BLE recovery audit performed after
> Phase C of the thin-fork architecture initiative (see
> `docs/upstream-sync-architecture.md`).
>
> **Read this as history, not as the current implementation contract:**
>
> - This document predates Phases A, B, and C of the thin-fork architecture
>   (`docs/upstream-sync-architecture.md`). It describes a codebase shape that no
>   longer exists on `develop` in several important ways — most notably, it assumes
>   `CrossPointSettings::bleEnabled` and a `SettingsList.h` row for BLE, both of
>   which Phase B relocated to `MidadAppSettings`/`MidadSettingsList`, and some
>   entries below describe a persistent BLE toggle design that a later commit on
>   the same stale branch itself already superseded with a dedicated pairing-screen
>   redesign (no persisted setting at all).
> - Some implementation details described here are **obsolete** and must not be
>   copied back into `develop` as-is. Treat every code reference, file path, and
>   architectural claim below as "true when written," not "true today."
> - This document must **not** be treated as the current BLE architecture or a
>   source of ready-to-merge code. It is retained purely for the design reasoning,
>   root-cause debugging narratives, and hardware-validation history it contains —
>   value that would otherwise be lost if the stale branch is ever deleted.
> - **Current BLE recovery decisions live in
>   [`docs/ble-recovery-plan.md`](../ble-recovery-plan.md).** Consult that document
>   for what is actually planned, what already exists on `develop`, and which parts
>   of this history are being recovered, reimplemented, or discarded.
> - The content below this banner is preserved unmodified from the source file —
>   including its own internal claims, corrections, and self-contradictions (e.g.
>   an early "validated end-to-end" claim that a later entry in this same document
>   retracts). No historical content has been edited, reordered, or removed.
>
> ---
