...
**Format** (TSV, `#`-comment and blank lines ignored): `PATH<TAB>UPSTREAM_COMMIT_SHA<TAB>BASE_BLOB_SHA<TAB>APPROVED_HEAD_BLOB_SHA<TAB>NOTE`. `BASE_BLOB_SHA` means the *effective* base for that path — the adopted-upstream blob above when one exists, otherwise `develop`'s own blob for the path.
