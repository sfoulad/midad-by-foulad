...
void TextBlock::render(const GfxRenderer& renderer, const int fontId, const int x, const int y) const {
  ...
  int latinBaselineShift = 0;
  {
    ...
    for (uint16_t i = 0; i < numWords && !lineHasArabic; i++) {
      lineHasArabic = ScriptDetector::containsArabic(wordText(i));
    }
    if (lineHasArabic) {
  ...
  for (uint16_t i = 0; i < numWords; i++) {
    ...
    if (latinBaselineShift != 0 && !ScriptDetector::containsArabic(word)) {
    ...
    if (boundary > 0) {
      ...
      renderer.drawText(fontId, drawX, wordY, word, true, currentStyle, baseDir, kashidaExtraPx(i));
    }
