...
void ParsedText::extractLine(const size_t breakIndex, const int pageWidth, const std::vector<uint16_t>& wordWidths,
  ...
  // extraStartOffset/extraEndOffset reserve space for any ruby group at the line's edges.
  ...
  // Upstream's focusBoundaryAt/measureFocusPrefixAdvance model keeps each word
  // as its own TextBlock entry (no merging of a focus-suffix word's text into
  // its preceding bold-prefix entry) -- the byte boundary alone tells render
  // where to switch style within one entry. That means lineWords/lineXPos/
  // lineWordStyles keep their original indices, so kashidaExtraPxFinal (built
  // against those same indices above) threads straight through unmodified --
  // no re-indexing, and no dropped kashida share for a suffix word the way
  // the old merge-based approach needed (see the !lineHasFocusSplit branch
  // above for the same hasAnyKashida-gating pattern).
