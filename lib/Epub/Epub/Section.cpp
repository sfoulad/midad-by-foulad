...
namespace {
// FORK VERSION HISTORY (this fork's own numbering, continued past upstream's):
// v29-v33: successive Arabic layout fixes (shaping/measurement correctness, Arabic
// line heights, RTL text-align resolution, no mid-word Arabic hyphenation, per-size
// Arabic word spacing) -- each changed layout without touching a cache-key field, so
// the version bump was the invalidation mechanism. See git history for details.
// v34: the resolved Arabic reading font id joins the header cache key. Arabic text
// dispatches to its own font (family + arabicFontSize), so changing the Arabic Font
// Size left the Latin fontId untouched and cached layouts kept "matching" -- an open
// Arabic book never re-laid-out at the new size, even across restarts (user report).
// v35: upstream merge -- TextBlock word data stored as one flat arena (offset table +
...
// first render).
// v40: upstream merge -- <ruby>/<rt> support (rubyTexts stored per-TextBlock,
// serialized alongside the kashida array; word-gap suppression now scoped to
// source-glued tokens only so ruby element boundaries no longer collapse
// Hangul word spaces); ReaderRenderSpec now carries the header fields instead
// of bare Section members (arabicFontId is derived from spec.fontId, not
...
void Section::writeSectionFileHeader(const ReaderRenderSpec& spec) {
  ...
  static_assert(HEADER_SIZE == sizeof(SECTION_FILE_VERSION) + sizeof(spec.fontId) + sizeof(int) /*arabicFontId*/ +
  ...
  // The Arabic font this layout was measured with (see SECTION_FILE_VERSION v34).
  serialization::writePod(file, renderer.getArabicFontIdFor(spec.fontId));
...
bool Section::loadSectionFile(const ReaderRenderSpec& spec) {
  ...
  bool filePartial = false;
  {
    ...
    serialization::readPod(file, fileArabicFontId);
    ...
    if (spec.fontId != fileFontId || renderer.getArabicFontIdFor(spec.fontId) != fileArabicFontId ||
...
bool Section::buildSomeMore(const int maxPages, const unsigned long budgetMs) {
  ...
  for (;;) {
    ...
    // paragraph, so the overshoot past the budget stays small).
