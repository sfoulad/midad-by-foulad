...
#include "Epub/parsers/LineHeightMath.h"
...
void ChapterHtmlSlimParser::flushPendingAnchor() {
  ...
  if (std::find(tocAnchors.begin(), tocAnchors.end(), pendingAnchorId) != tocAnchors.end()) {
    if (currentPage && !currentPage->elements.empty()) {
      ...
      if (!currentPage) {
        LOG_ERR("EHP", "Failed to create page after TOC anchor page break");
        return;
      }
      currentPageNextY = 0;
  ...
  for (size_t lineIndex = 0; lineIndex < maxLineCount; ++lineIndex) {
    ...
    for (size_t column = 0; column < columnCount; ++column) {
      if (lineIndex < tableCellLines[column].size()) {
        // Per-cell, not the row's flat Latin default: an Arabic cell line needs
        // computeLineHeight's max(latin, arabic) comparison, the same formula
        // addLineToPage uses below -- otherwise this page-fit check can accept
        // a row that addLineToPage's own (correct) height then pushes onto the
        // next page, splitting the row across pages.
        const auto& cellLine = tableCellLines[column][lineIndex];
        rowLineHeight = std::max<int16_t>(
            rowLineHeight, static_cast<int16_t>(computeLineHeight(*cellLine) +
                                                cellLine->getRubyShift(renderer.getFontAscenderSize(fontId))));
      }
    }
...
int ChapterHtmlSlimParser::computeLineHeight(const TextBlock& line) const {
  ...
  for (uint16_t i = 0; i < line.wordCount() && !hasArabic; i++) {
    hasArabic = ScriptDetector::containsArabic(line.wordText(i));
  }
  const int arabicLineHeight = hasArabic ? renderer.getLineHeight(renderer.getResolvedArabicFontId(fontId)) : 0;
  return effectiveLineHeight(latinLineHeight, arabicLineHeight, hasArabic, lineCompression);
}

void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line, const uint32_t visibleOffset) {
  const int lineHeight = computeLineHeight(*line) + line->getRubyShift(renderer.getFontAscenderSize(fontId));

  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page for line");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  if (currentPageNextY + lineHeight > viewportHeight) {
    ...
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page after line break");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
...
void ChapterHtmlSlimParser::makePages() {
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      LOG_ERR("EHP", "Failed to create page in makePages");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
