...
def generate_keys_header(
    ...
    # (English first, then by native name -- so the on-device picker lists "Deutsch",
    # "Español", "中文" etc. in the order a reader of that language would expect, not by
    # the internal two-letter code, which for many languages sorts nothing like the name)
    ...
    lines.append("// V1 languages no longer bundled in this build fall back to EN.")
    lines.append("constexpr Language V1_LANGUAGES[] = {")
    lines.append(
        "    "
        + ", ".join(f"Language::{c}" if c in languages else "Language::EN" for c in v1_codes)
        + ","
    )
    lines.append("};")
    lines.append(
