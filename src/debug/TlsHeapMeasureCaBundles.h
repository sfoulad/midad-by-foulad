...
// Root CA bundles for the TLS heap measurement harness (see
// TlsHeapMeasureActivity.h/.cpp). Sourced verbatim from the Mozilla CA bundle
// already vendored into this repo's PlatformIO dependency tree at
// .pio/libdeps/default/WebSockets/examples/esp32_pio/WebSocketClientSSLBundle/cacrt_all.pem
// (229,654 bytes on disk, 147 certs, "Certificate data from Mozilla as of:
// Mon Mar 11 15:25:27 2024 GMT" per that file's own header comment) -- no new
// certificates were fetched or hand-authored for this harness. Only that
// file's leading '##' comment block and the per-cert "Name\n====" divider
// lines (not valid PEM) were stripped; every BEGIN/END CERTIFICATE block
// below is copied byte-for-byte from the source file.
//
// static constexpr, not just static const: flash-resident per CLAUDE.md's
// "constexpr First" rule -- ~236 KB combined across the three bundles must
// never be copied into DRAM. Byte counts are computed from sizeof() below
...
// 12 certs covering the most common real-world issuers actually present in
// the source file: ISRG Root X1 (Let's Encrypt), DigiCert Global Root CA,
// DigiCert Global Root G2, GlobalSign Root CA - R3, Amazon Root CA 1, GTS
// Root R1 (Google Trust Services), Entrust Root Certification Authority -
// G2, USERTrust RSA Certification Authority and COMODO RSA Certification
// Authority (Sectigo/Comodo, current + legacy), Microsoft RSA Root
// Certificate Authority 2017, SSL.com TLS ECC Root CA 2022, and Sectigo
// Public Server Authentication Root R46. Verified (openssl s_client
// -showcerts) that example.com's live chain -- leaf -> "Cloudflare TLS
// Issuing ECC CA 3" -> "SSL.com TLS Transit ECC CA R2" -> trust anchor
// "SSL.com TLS ECC Root CA 2022" -- resolves against this bundle.
constexpr size_t kProductionCertCount = 12;
static constexpr char kProductionCaBundle[] =
    R"PEMCERT(-----BEGIN CERTIFICATE-----
...
constexpr size_t kProductionBundleBytes = sizeof(kProductionCaBundle) - 1;

// --- stress bundle -------------------------------------------------------
// The entire 147-cert source file, verbatim (comment/divider lines stripped
// as noted above; PEM cert content untouched). Tested against example.com,
// same as the production bundle -- any real HTTPS site works here since the
// point is the parse cost of the full set, not which single root actually
// gets used to validate the chain.
constexpr size_t kStressCertCount = 147;
static constexpr char kStressCaBundle[] =
    R"PEMCERT(-----BEGIN CERTIFICATE-----
MIIDdTCCAl2gAwIBAgILBAAAAAABFUtaw5QwDQYJKoZIhvcNAQEFBQAwVzELMAkGA1UEBhMCQkUx
GTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2ExEDAOBgNVBAsTB1Jvb3QgQ0ExGzAZBgNVBAMTEkds
b2JhbFNpZ24gUm9vdCBDQTAeFw05ODA5MDExMjAwMDBaFw0yODAxMjgxMjAwMDBaMFcxCzAJBgNV
...
constexpr size_t kStressBundleBytes = sizeof(kStressCaBundle) - 1;
