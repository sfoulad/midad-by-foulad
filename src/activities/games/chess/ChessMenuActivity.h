...
// Chess landing screen: new game, resume, openings trainer and the record.
...
class ChessMenuActivity final : public Activity {
 ...
 private:
  ...
  enum class Row { NewGame, Continue, Openings, Statistics };

  ButtonNavigator buttonNavigator_;
  int selectedIndex_ = 0;
  bool hasSaved_ = false;

  int rowCount() const { return hasSaved_ ? 4 : 3; }
