...
// Derives rowNames/rowExtensions/rowItems from `files` (plus the synthetic
// File Transfer row, when hasTransferRow()). Called whenever `files` changes
// (end of loadFiles()) so buildScreen() can reuse the cached rows on every
// repaint instead of re-deriving a name/extension string (and a ListItem) per
...
void FileBrowserActivity::rebuildRowItems() {
  ...
  const size_t rowCount = files.size() + (transferRow ? 1 : 0);
  rowNames.resize(rowCount);
  rowExtensions.resize(rowCount);
  rowItems.clear();
  rowItems.reserve(rowCount);

  if (transferRow) {
    rowNames[0] = tr(STR_FILE_TRANSFER);
    rowExtensions[0].clear();
    fui::ListItem item;
    item.label = rowNames[0].c_str();
    item.icon = listIconFor(UIIcon::Transfer);
    item.actionValue = 0;
    rowItems.push_back(item);
  }

  for (size_t i = 0; i < files.size(); i++) {
    const size_t row = transferRow ? i + 1 : i;
    rowNames[row] = getFileName(files[i]);
    rowExtensions[row] = getFileExtension(files[i]);
    fui::ListItem item;
    item.label = rowNames[row].c_str();
    if (!rowExtensions[row].empty()) item.value = rowExtensions[row].c_str();
...
void FileBrowserActivity::onEnter() {
  ...
  // If Confirm was held while this activity opened (typical when launched via a
  // long-press elsewhere), ignore its release -- otherwise we'd immediately
  // auto-activate whatever is at the default selection.
  ...
  if (!root) {
    ...
    nav.selected = static_cast<int>(findEntry(fileName)) + (hasTransferRow() ? 1 : 0);
  } else {
    loadFiles();
  }
}
...
void FileBrowserActivity::activateSelected(const bool forceDelete) {
  // The synthetic File Transfer row (see hasTransferRow()) launches file
  // transfer regardless of hold duration -- it has no delete action.
  if (hasTransferRow() && nav.selected == 0) {
  ...
  const size_t fileIndex = static_cast<size_t>(nav.selected) - (hasTransferRow() ? 1 : 0);
...
bool FileBrowserActivity::handleButtons() {
  // Swallow a stray Confirm release left over from before this activity
  // opened (see the comment in onEnter()) instead of activating a row.
    ...
    if (mappedInput.getHeldTime() < GO_HOME_MS) {
      if (basepath != "/") {
...

        {
          ...
          nav.selected = static_cast<int>(findEntry(dirName)) + (hasTransferRow() ? 1 : 0);
          nav.top = 0;
          nav.follow(listCount());
        }
...
void FileBrowserActivity::drawFooter() {
  ...
  // Bounds-check nav.selected before indexing files[]: listCount() includes the synthetic
  // row, so it can exceed files.size() while nav.selected still points past files' end.
  const bool selectingFirmwareFile = mode == Mode::PickFirmware && !files.empty() && nav.selected >= 0 &&
                                     nav.selected < static_cast<int>(files.size()) && files[nav.selected].back() != '/';
