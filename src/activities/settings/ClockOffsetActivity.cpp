...
void ClockOffsetActivity::render(RenderLock&&) {
  ...
  if (halClock.isAvailable()) {
    ...
    if (halClock.formatTime(timeBuf, sizeof(timeBuf), encoded, SETTINGS.clockFormat == 1)) {
      ...
      // is 26 bytes in Russian, 24 in Arabic and Ukrainian -- UTF-8 multibyte sequences
      // corrupt the last glyph if truncated instead of just clipping ASCII. See
      // ClockSyncActivity.
      char preview[64];
  ...
  const auto labels =
      mappedInput.mapLabels(tr(STR_BACK), tr(STR_NEXT_FIELD), tr(STR_DIR_UP), tr(STR_DIR_DOWN), /*rtlSwap=*/false);
