...
// extraction on first enable) so both SettingsActivity and the touch
// Settings presentation share it instead of re-deriving it. Returns true if
// the caller should repaint immediately (the Quran extraction popup was
// shown and needs to be cleared).
...
// Localized "current value" text for a setting's list row -- shared by
// SettingsActivity's legacy list render and the touch Settings presentation
// so the two can never show a different value for the same setting.
