...
inline freeink::ui::ThemeTokens uiThemeTokens(const freeink::ui::GfxRendererTarget& target) {
  ...
  // inward past the covered side. Lists render in the portrait UI frame, so
  // the panel-native portrait margins (GfxRenderer::VIEWABLE_MARGIN_*) apply directly.
  tokens.listScrollInset = static_cast<int16_t>(metrics.listScrollSide == 1 ? GfxRenderer::VIEWABLE_MARGIN_LEFT
                                                                            : GfxRenderer::VIEWABLE_MARGIN_RIGHT);
